...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if(eax == 0x4fffffff){
		...
		//high 32 bit of total time spent processing all exits
		ebx = (atomic64_read(&exit_time) >> 32);
		//low 32 bit of total time spent processing all exits
		ecx = (atomic64_read(&exit_time) & 0xFFFFFFFF);
		printk("exit_count: %x, exit_time: %11x", atomic64_read(&exit_count), atomic64_read(&exit_time) );
